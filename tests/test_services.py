...
def test_fear_greed_ttl_is_one_hour() -> None:
    assert FEAR_GREED_TTL_SECONDS == 60 * 60
...
def test_get_etf_flow_sorts_sosovalue_rows_before_trimming(monkeypatch, tmp_path) -> None:
    def fake_get(url: str, **kwargs) -> FakeResponse:
        return FakeResponse(status_code=503)
    ...
    rows.extend(
        {"date": f"2025-05-{day:02d}", "totalNetInflow": 1_000_000.0}
        for day in range(1, 31)
    )

    def fake_post(url: str, **kwargs) -> FakeResponse:
        return FakeResponse({"code": 0, "msg": None, "data": rows})
    ...
    assert payload["latest_date"] == "2026-06-12"
    assert payload["latest_net_flow_usd"] == 85_900_000.0
    assert len(payload["flow_history"]) == 30
...
def test_get_etf_flow_ignores_sosovalue_same_day_zero_placeholder(
    ...
    def fake_post(url: str, **kwargs) -> FakeResponse:
        return FakeResponse({
            ...
            "data": {
                "list": [
                    {"date": "2026-05-27", "totalNetInflow": -193_000_000.0},
                    {"date": "2026-05-28", "totalNetInflow": 0.0},
                ]
            },
        })
    ...
    monkeypatch.setattr(
        "btc_dashboard.services._utc_now_dt",
        lambda: datetime(2026, 5, 28, tzinfo=UTC),
    )
...
def test_get_etf_flow_ignores_sosovalue_previous_day_zero_placeholder(
    monkeypatch,
    tmp_path,
) -> None:
    def fake_get(url: str, **kwargs) -> FakeResponse:
